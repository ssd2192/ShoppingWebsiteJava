...
public class DAORateTwoWatch {
     public List<UserListaccessories> fetch(){
try{
Connection conn=MyConnection.OpenConnection();
PreparedStatement stmt=conn.prepareStatement("select * from PRODUCTS where UPPER(CATEGORY)=UPPER('watch') AND (UPPER(PRICE)>2000 AND UPPER(PRICE)<=4000)");      
